          ...
          Future {
            ...
            if (collection.getMembers.contains(user) ||  SitePermissions.userHasPermission(user, "admin"))
              Ok(views.html.collections.view(collection, Json.toJson(collection.getLinkedCourses.map(_.toJson)).toString, 
                Json.toJson(User.findUsersByUserIdList(CollectionMembership.getExceptionsByCollection(collection).map(_.userId)).map(_.toJson)).toString, collection.getCoowners.map(_.toJson).toString))
            else
              Errors.forbidden
          }
        }

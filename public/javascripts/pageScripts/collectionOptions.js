	...
	return {
        ...
        contains: function(course) {
            if (typeof course === "string") {
                return courses.some(function(c){return c.name === course;});
            } else if (typeof course === "object") {
                return courses.some(function(c){return c.name === course.name;});
            } else return false;
        },
...
.controller("removeCourseController", function($scope, linkedCourses) {
	...
	$scope.submit = function() {
		$.ajax("/collection/" + $scope.$parent.collectionId + "/unlinkCourses", {
			...
			dataType: "json",
			contentType: "application/json; charset=utf-8",
			data: JSON.stringify($scope.markedCourses.map(function(c){return c.name;})),
			success: function(value) {
				// turn off spinner...
                // TODO: check for invalid json
                value = JSON.parse(value);
